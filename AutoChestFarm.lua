-- ✅ CrazyHub | Full Script with Auto Fruit Store, Duplicate Drop, Logging, UNC Compatible
...
local guiLabel, lastRollTime = nil, 0
...
local function isFruitStored(fruitName)
    local inv = ReplicatedStorage:FindFirstChild("Remotes"):FindFirstChild("CommF_")
    if inv then
        local owned = inv:InvokeServer("GetFruits")
        for _, f in pairs(owned or {}) do
            if f:lower() == fruitName:lower() then
                return true
            end
        end
    end
...
local function storeFruit(fruit)
    ...
    if remote then
        pcall(function()
            remote:InvokeServer("StoreFruit", fruit)
            sendWebhook("Fruit Stored", "Stored **" .. fruit .. "** in inventory.")
            OrionLib:MakeNotification({ Name = "Stored", Content = fruit .. " saved.", Time = 3 })
        end)
    end
end
    ...
    while isRunning do
        ...
        for _, tool in pairs(workspace:GetDescendants()) do
            ...
            if tool:IsA("Tool") and tool:FindFirstChild("Handle") then
                ...
                if name:lower():find("fruit") or name:lower():find("chalice") or name:lower():find("darkness") or name:lower():find("fist") then
                    sendWebhook("Item Collected", "**" .. name .. "** collected")
                    task.wait(1)
                    if isFruitStored(name) then
                        OrionLib:MakeNotification({ Name = "Dropped", Content = name .. " already owned. Dropping!", Time = 3 })
                        dropTool(tool)
                        sendWebhook("Duplicate Dropped", "**" .. name .. "** was dropped (already stored).")
                    else
                        storeFruit(name)
                    end
                end
...
local function loadMainUI()
    ...
    MainTab:AddParagraph("Teleport", "Change Sea (Unlocked Only)")
    MainTab:AddButton({
        Name = "Go to 2nd Sea",
        Callback = function()
            local r = ReplicatedStorage:FindFirstChild("Remotes"):FindFirstChild("CommF_")
            if r then r:InvokeServer("TravelDressrosa") end
        end
    })
    MainTab:AddButton({
        Name = "Go to 3rd Sea",
        Callback = function()
            local r = ReplicatedStorage:FindFirstChild("Remotes"):FindFirstChild("CommF_")
            if r then r:InvokeServer("TravelZou") end
        end
    })
end
